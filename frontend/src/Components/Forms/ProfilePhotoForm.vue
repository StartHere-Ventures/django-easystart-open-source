...
export default {
  ...
  methods: {
    ...
    validateExtension(){
      var allow = false;
      var ext = (this.photo.name.substring(this.photo.name.lastIndexOf("."))).toLowerCase();
    ...
    onSubmit(event) {
      ...
      this.$inertia.post(this.url, {"photo": this.photo}, {
        ...
        onFinish: () => {
          ...
          if(this.errors){
            if (this.errors["photo"]) {
              this.textError = this.errors["photo"];
              this.error = true
            }
          }
