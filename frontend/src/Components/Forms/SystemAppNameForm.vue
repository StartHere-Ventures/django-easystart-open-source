  ...
  <div>
    ...
    >
      <div class="sm:grid sm:grid-cols- sm:gap-4">
        <div>
          <label for="app-name" class="sr-only">{{ $_("App name") }}</label>
          <basic-input
            name="app-name"
            :value="form.appName"
            :error="fieldsErrors.appName"
            :text-error="fieldsErrors.appName ? textErrors.appName: ['']"
            @input-return="value => form.appName = value"
            @keypress="fieldsErrors.appName = false"
            @blur="checkFieldsErrors('appName')"
            @change="checkFieldsErrors('appName')"
          />
        </div>
      </div>
...
export default {
  ...
  props: {
    ...
    url: {
      type: String,
      default: () => '/manage/settings/system/change/app-name'
    }
  },
  emits: [
    'close-update-name',
  ...
  methods: {
    ...
    fieldsValidate() {
      let valid = true;
      this.checkFieldsErrors("appName");
